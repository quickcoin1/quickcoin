...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 QIC at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zQIC send
enum ZerocoinSpendStatus {
    ZQIC_SPEND_OKAY = 0,                            // No error
    ZQIC_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZQIC_WALLET_LOCKED = 2,                         // Wallet was locked
    ZQIC_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZQIC_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZQIC_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZQIC_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZQIC_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZQIC_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZQIC_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZQIC_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZQIC_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZQIC_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZQIC_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZQIC_SPENT_USED_ZQIC = 14                       // Coin has already been spend
};
...
public:
    ...
    CAmount GetAnonymizableCredit(bool fUseCache = true) const
    {
        ...
        for (unsigned int i = 0; i < vout.size(); i++) {
            ...
            if (fMasterNode && vout[i].nValue == 2750 * COIN) continue; // do not count MN-like outputs
    ...
    CAmount GetLockedCredit() const
    {
        ...
        for (unsigned int i = 0; i < vout.size(); i++) {
            ...
            if (fMasterNode && vout[i].nValue == 2750 * COIN) {
                nCredit += pwallet->GetCredit(txout, ISMINE_SPENDABLE);
            }
