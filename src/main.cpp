...
#if defined(NDEBUG)
#error "Quickcoin cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        ...
        nSubsidy = 0.2 * COIN;
    } else if (nHeight < 4503) {
        nSubsidy = 0.4 * COIN;
    } else if (nHeight < 76504) {
        nSubsidy = 50 * COIN;
    } else if (nHeight < 522905) {
        nSubsidy = 3.5 * COIN;
    } else {
        nSubsidy = 0.4 * COIN;
    }
...
bool IsInitialBlockDownload()
{
    ...
    bool state = (chainActive.Height() < pindexBestHeader->nHeight - 24 * 6 ||
                  pindexBestHeader->GetBlockTime() < GetTime() - 6 * 60 * 60) && chainActive.Height() > 3; // ~144 blocks behind -> 2 x fork detection time
    if (!state)
        lockIBDState = true;
    return state;
}
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Quickcoin
...
{
    RenameThread("quickcoin-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZQICMinted()
{
...
void RecalculateZQICSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zQIC supply
...
bool RecalculateQICSupply(int nHeightStart)
{
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert QIC to zQIC
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Quickcoin: We use certain sporks during IBD, so check to see if they are
